...
def populate_db(
    ...
    tenants = [
        Tenant(name=name, tenant_type=tenant_type) for name, tenant_type in cutomer_data
    ]
    session.add_all(tenants)
    session.commit()
    result = session.execute(sa.select(Tenant))
    db_tenants = result.scalars().all()
    tenant_dict = {c.name: c.id for c in db_tenants}

    result = session.execute(sa.select(Group))
    db_groups = result.scalars().all()
    group_dict = {g.name: g.id for g in db_groups}
    ...
    for tenant in db_tenants:
        ...
        users.append(
            User(
                ...
                tenant_id=tenant_dict.get(tenant.name),
                default_group_id=group_dict.get(tenant.name),
            )
        )
    ...
    return {"users": user_dict, "tenants": tenant_dict, "groups": group_dict}
